...
contract User is StringFeature {
    ...
    function createUser(bytes16 username, bytes title, bytes intro) public returns(bool success) {
        require(!hasUser(msg.sender));
        require(!usernameTaken(username));
        addresses.push(msg.sender);
        usernames.push(username);
        string memory ipfsHashStr = addString(string(title), "|||");
        ipfsHashStr = addString(ipfsHashStr, string(intro));
        bytes memory ipfsHash = bytes(ipfsHashStr);
